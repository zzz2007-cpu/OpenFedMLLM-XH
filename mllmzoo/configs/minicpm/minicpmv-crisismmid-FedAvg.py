...
_runtime = resolve_runtime_overrides(
    default_data_path="./data/crisis-mmd/minicpmv_data/partition-alpha1.0-clt10",
    default_eval_path="./data/crisis-mmd/minicpmv_data/test.json",
    default_output_dir="./mllmzoo/output/minicpmv_crisismmid_fedavg",
    default_fed_alg="fedavg",
...
if __name__ == "__main__":
    ...
    def _maybe_redirect_rank_logs(output_dir):
        ...
        if tee:
            ...
            sys.stderr = log_fp
        print(
            f"[minicpmv_crisismmid_fedavg] rank log redirected: rank={rank}, local_rank={local_rank}, "
    ...
    if exp_args.eval_args.eval_data_path and not os.path.exists(exp_args.eval_args.eval_data_path):
        print(
            "[minicpmv_crisismmid_fedavg] WARNING: eval_data_path not found: "
            f"{exp_args.eval_args.eval_data_path}. Per-round eval will be skipped."
        )

    print(
        "[minicpmv_crisismmid_fedavg] data_path="
    ...
    class PrintMetricsHook(FederatedHook):
        ...
        每轮结束后打印：
          - 轮次、各客户端 loss
          - GPU 显存占用（若有 CUDA）
          - 评估 Acc / F1（若配置了 eval_data_path 且文件存在）
        不依赖复杂的 EvalHook 系统。
        """
        ...
        def on_aggregate_end(self, context):
            ...
            # ── 通信量统计 ─────────────────────────────────────────────────
            # 公式：每轮通信 = 2(上传+下载) × 参与客户端数 × LoRA参数量 × 2字节(bf16)
