...
exp_args = dict(
    ...
    training_args=dict(
        output_dir="./mllmzoo/output/minicpmv_crisismmid_fednova_quick_meaningful",
...
if __name__ == "__main__":
    ...
    def _maybe_redirect_rank_logs(output_dir):
        ...
        if tee:
            ...
            sys.stderr = log_fp
        print(
            f"[minicpmv_crisismmid_fednova_quick] rank log redirected: rank={rank}, local_rank={local_rank}, "
    ...
    if exp_args.eval_args.eval_data_path and not os.path.exists(exp_args.eval_args.eval_data_path):
        print(
            "[minicpmv_crisismmid_fednova_quick] WARNING: eval_data_path not found: "
            f"{exp_args.eval_args.eval_data_path}. Per-round eval will be skipped."
        )

    print(
        "[minicpmv_crisismmid_fednova_quick] data_path="
        f"{exp_args.data_args.data_path}, eval_data_path={exp_args.eval_args.eval_data_path}"
    )
    print(
        "[minicpmv_crisismmid_fednova_quick] profile="
    ...
    if exp_args.training_args.max_steps > 0:
        ...
        print(
            "[minicpmv_crisismmid_fednova_quick] NOTE: max_steps>0 means each client uses at most "
