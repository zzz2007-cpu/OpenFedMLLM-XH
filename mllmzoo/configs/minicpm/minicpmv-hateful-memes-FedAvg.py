...
def _resolve_hateful_runtime():
    ...
    eval_path = Path(os.environ.get("OPENFED_HATEFUL_MEMES_EVAL_PATH", str(root / "dev_seen.jsonl")))
    output_dir = os.environ.get(
        "OPENFED_HATEFUL_MEMES_OUTPUT_DIR",
        f"./outputs/checkpoints/minicpmv_hateful_memes_{stat_setting}_{modal_setting}",
    )
    return {
        "root": str(root),
...
exp_args = dict(
    ...
    training_args=dict(
        ...
        gradient_accumulation_steps=_get_env_int("OPENFED_GRAD_ACCUM_STEPS", 16),
        learning_rate=_get_env_float("OPENFED_LR", 3e-5),
        bf16=os.environ.get("OPENFED_BF16", "1") == "1",
        fp16=os.environ.get("OPENFED_FP16", "0") == "1",
        logging_steps=_get_env_int("OPENFED_LOGGING_STEPS", 20),
        save_steps=_get_env_int("OPENFED_SAVE_STEPS", 1000),
        save_total_limit=_get_env_int("OPENFED_SAVE_TOTAL_LIMIT", 1),
        remove_unused_columns=False,
        report_to="none",
        model_max_length=_get_env_int("OPENFED_MODEL_MAX_LENGTH", 1024),
        max_slice_nums=_get_env_int("OPENFED_MAX_SLICE_NUMS", 9),
        vision_batch_size=_get_env_int("OPENFED_VISION_BATCH_SIZE", 13),
        llm_type="minicpm",
    ...
    lora_args=dict(
        lora_r=_get_env_int("OPENFED_LORA_R", 8),
        lora_alpha=_get_env_int("OPENFED_LORA_ALPHA", 16),
    ...
    fed_args=dict(
        ...
        num_rounds=_get_env_int("OPENFED_NUM_ROUNDS", 20),
        num_clients=_get_env_int("OPENFED_NUM_CLIENTS", 10),
        sample_clients=_get_env_int("OPENFED_SAMPLE_CLIENTS", 5),
        ...
        save_model_freq=_get_env_int("OPENFED_SAVE_MODEL_FREQ", 5),
        prox_mu=0.01,
        mu_w=0.1,
        s_layer=4,
        fedopt_tau=1e-3,
    ...
    eval_args=dict(
        ...
        strict_image_path=_get_env_bool("OPENFED_HATEFUL_MEMES_STRICT_IMAGE_PATH", False),
        eval_freq=1,
        max_new_tokens=_get_env_int("OPENFED_EVAL_MAX_NEW_TOKENS", 8),
        max_samples=_get_env_int("OPENFED_EVAL_MAX_SAMPLES", 500),
        startup_eval_full=False,
        final_eval_full=True,
    ),
    run_args=dict(
        mode=os.environ.get("OPENFED_RUN_MODE", "federated"),
        enable_final_eval=os.environ.get("OPENFED_ENABLE_FINAL_EVAL", "1") == "1",
    ),
)
